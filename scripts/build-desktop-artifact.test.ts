...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it.effect("embeds only an explicitly configured Turbo fork update feed", () =>
    Effect.gen(function* () {
      const latestConfig = yield* resolveTurboGitHubPublishConfig("latest");
      const nightlyConfig = yield* resolveTurboGitHubPublishConfig("nightly");
      ...
      Effect.provide(
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: { T3CODE_DESKTOP_UPDATE_REPOSITORY: "gfsaaser24/t3code" },
          }),
        ),
      ),
    ),
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3 Turbo 1.2.3 Installer",
