...
export const resolveNightlyReleaseMetadata = (
  ...
  // T3 Turbo Fork Policy: nightly releases are named "T3 Turbo MM-DD-YY"; the
  // date is the 11:00 PM ET upstream-ingestion cutoff. Every other prerelease
  // train keeps upstream's loud label, which is what stops someone installing
  // a maintainer test build by accident.
  ...
  return {
    ...
    name:
      channel === "nightly"
        ? `T3 Turbo ${policyDate}`
        : `T3 Code ${CHANNEL_RELEASE_LABELS[channel]} ${version} (${shortSha})`,
    shortSha,
  };
};
