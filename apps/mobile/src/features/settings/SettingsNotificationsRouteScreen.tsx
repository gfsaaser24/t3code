  ...
  const handleDeviceNotificationsChange = useCallback(
    (enabled: boolean) => {
      ...
      Alert.alert(
        ...
        `Open system Settings to disable notifications for ${MOBILE_PRODUCT_NAME}.`,
        [
          { text: "Cancel", style: "cancel" },
          { text: "Open Settings", onPress: () => void Linking.openSettings() },
        ],
              ...
              onPress={() => {
                void openAndroidLiveUpdateSettings().catch(() => {
                  Alert.alert(
                    ...
                    `Open Android Settings, select ${MOBILE_PRODUCT_NAME}, then enable Live Updates in Notifications.`,
                  );
                });
              }}
            />
