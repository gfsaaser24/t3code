...
// T3 Turbo re-versions an ingested upstream nightly with a trailing
// `.turbo.<run>` or `.turbo.<date>.<run>`; those are still nightly builds.
...
const PRERELEASE_VERSION_PATTERN =
  /^[^-+]+-(?:nightly|preview)\.\d{8}\.\d+(?:\.turbo\.(?:\d{8}\.\d+|\d+))?$/u;
