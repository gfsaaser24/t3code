// This file mostly exists because we want dev mode to say "T3 Turbo (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
const APP_DISPLAY_NAME = isDevelopment ? "T3 Turbo (Dev)" : "T3 Turbo (Alpha)";
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSDocumentsFolderUsageDescription: "T3 Turbo reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the T3 Turbo name.
