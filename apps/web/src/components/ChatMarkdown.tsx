...
const MarkdownFileLink = memo(function MarkdownFileLink({
  ...
  const handleOpenWithSystemDefault = useCallback(() => {
    if (!onOpenWithSystemDefault) return;
    void (async () => {
      try {
...
function useChatMarkdownState({
  ...
  // Turbo markdown-preview seam: Ctrl+Shift+click opens with the OS default
  // app instead of the preferred editor (resolveMarkdownFileLinkGesture).
      ...
      if (environmentId === null) {
        return Promise.resolve(
          AsyncResult.failure<unknown, unknown>(
            Cause.fail(new Error("No environment is selected.")),
          ),
        );
      }
      return openPath({
    ...
    (
      ...
      return (
        <MarkdownFileLink
          ...
          {...(canUseShellActions ? { onOpenWithSystemDefault: openWithSystemDefault } : {})}
  ...
  pre: function MarkdownPre({ node, children, ...props }) {
    ...
    return (
      <MarkdownCodeBlock
        ...
        theme={resolvedTheme}
      >
        {/* Turbo: a streaming fence shows a line-counted placeholder inside the
            block frame and is highlighted exactly once, when the message
            completes. The wrapper stays INSIDE MarkdownCodeBlock so the copy
