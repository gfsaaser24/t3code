...
>(threads: readonly T[]): T[] {
  ...
  const decorated = threads.map((thread) => ({
    thread,
    anchorMs: activeThreadAnchorTimestampMs(thread),
  }));
  decorated.sort(
    (left, right) =>
      right.anchorMs - left.anchorMs || left.thread.id.localeCompare(right.thread.id),
  );
  return decorated.map((entry) => entry.thread);
}
...
>(threads: readonly T[]): T[] {
  // Decorate-sort: resolveSettledThreadTimestamp walks four candidates and parses
