...
export function getStandardThemeColors(appearance: ThemeAppearance): ThemeColors {
  if (appearance === "dark") {
    return (standardDarkThemeColors ??= decodeThemeColors(
      withRegionThemeRoles(T3_CODE_DARK_THEME_COLORS),
    ));
  }
  return (standardLightThemeColors ??= decodeThemeColors(
    withRegionThemeRoles(T3_CODE_LIGHT_THEME_COLORS),
  ));
}
...
export const T3_CHAT_THEME: ThemeDefinition = {
  ...
  appearance: "light",
  colors: decodeThemeColors(withRegionThemeRoles(T3_CHAT_LIGHT_COLORS)),
  variants: {
    dark: decodeThemeColors(withRegionThemeRoles(T3_CHAT_DARK_COLORS)),
  },
  sidebarArtwork: true,
};
