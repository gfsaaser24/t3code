...
// Turbo: agents running on Windows emit relative backslash destinations
// (docs\guide.md). The path candidate checks only understand forward slashes,
// so normalize them; drive, UNC, and scheme-qualified destinations keep theirs.
const WINDOWS_DRIVE_OR_UNC_OR_SCHEME = /^(?:[A-Za-z]:[\\/]|\\\\|[A-Za-z][A-Za-z0-9+.-]*:)/;
function normalizeRelativeBackslashes(href: string): string {
  if (!href.includes("\\") || WINDOWS_DRIVE_OR_UNC_OR_SCHEME.test(href)) return href;
...
export function resolveMarkdownFileLinkTarget(
  ...
  if (!href) return null;
  const target = parseMarkdownFileLink(normalizeRelativeBackslashes(href));
