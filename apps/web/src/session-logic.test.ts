...
describe("deriveActivePlanState", () => {
  it("orders plan snapshots by sequence while ignoring unrelated activities", () => {
    ...
    // Fork seam (unified-activity-order): derivations take pre-ordered input,
    // so the fixture is sorted here the way ChatView sorts once per thread.
    expect(
      deriveActivePlanState(sortThreadActivities(activities), TurnId.make("turn-1"))?.steps,
    ).toEqual([{ step: "Check", status: "completed", durationMs: 5_000 }]);
