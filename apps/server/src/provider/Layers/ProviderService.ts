...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const processRuntimeEvent = (
    ...
    Effect.gen(function* () {
      ...
      // T3 Turbo: keep the persisted session binding honest. Runs on every
      // event path (including the compaction short-circuits below), and is
      // best-effort - it can never fail event delivery.
      yield* syncBindingOnRuntimeEvent(source.adapter, canonicalEvent);
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in T3 Turbo settings.`,
          );
        }
