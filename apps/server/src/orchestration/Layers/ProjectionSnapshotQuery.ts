...
// Canonical activity order — mirror threadActivityOrder.ts and the SQL ORDER BY
// clauses above: null sequences sink, then createdAt, the lifecycle rank, and
// byte-order ids (not the ICU collator). Ids are passed alongside the row so
// both the decoded activities and the raw DB rows can share one comparator.
function activityLifecycleRank(kind: string): number {
  return kind.endsWith(".started")
    ? 0
...
interface CanonicalActivityOrderRow {
...
function compareCanonicalActivityOrder(
  left: CanonicalActivityOrderRow,
  leftId: string,
  right: CanonicalActivityOrderRow,
  rightId: string,
): number {
...
pending_approval_requests AS (
  ...
  const listProjectedThreadActivities = Effect.fn(
    ...
    return activities.toSorted((left, right) =>
      compareCanonicalActivityOrder(left, left.id, right, right.id),
    );
  });
          ...
          : Effect.all([
              ...
              Effect.map(([activityRows, pinnedActivityRows]) =>
                [
                  ...
                  .toSorted((left, right) =>
                    compareCanonicalActivityOrder(left, left.activityId, right, right.activityId),
                  )
