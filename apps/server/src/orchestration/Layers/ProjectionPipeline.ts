...
const makeOrchestrationProjectionPipeline = Effect.fn("makeOrchestrationProjectionPipeline")(
  function* () {
     ...
     * event. Like `runProjectorForEvent`, it collects attachment side-effects
     * and discards them: replay must not delete files, because `bootstrap`
     * prunes once after every projector has committed, off its own cursor.
     */
