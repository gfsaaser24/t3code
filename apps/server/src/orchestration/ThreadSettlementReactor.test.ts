...
describe("ThreadSettlementReactor", () => {
  ...
  it.effect("skips PR work on startup, timer and merge sweeps when settlement is disabled", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          const reactor = yield* ThreadSettlementReactor.ThreadSettlementReactor;
          // Automatic sweeps are muted for the boot delay, so step past it
          // before starting; the timer sweep below then has to clear the
          // minimum interval, and one periodic tick fires per clock adjustment.
          yield* TestClock.adjust(SWEEP_BOOT_DELAY);
          yield* reactor.start();
  ...
  it.effect("a project override settles only that project's inactive threads", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          const reactor = yield* ThreadSettlementReactor.ThreadSettlementReactor;
          // Step past the boot delay so the periodic tick from `start()` sweeps.
          yield* TestClock.adjust(SWEEP_BOOT_DELAY);
          yield* reactor.start();
          yield* Queue.take(fixture.settingsReads);
          yield* Deferred.succeed(fixture.activation, undefined);
  ...
  it.effect("skips a per-thread event sweep when settlement is disabled", () =>
    Effect.scoped(
      Effect.gen(function* () {
        yield* TestClock.setTime(Date.parse(NOW));
        const fixture = yield* makeHarness({
        ...
        yield* Effect.gen(function* () {
          ...
          // Per-thread event sweeps ride the same queue but are never
          // throttled, so the disabled check is the only thing stopping them.
          yield* fixture.publishEvent({
            sequence: 2,
            eventId: EventId.make("session-set"),
            aggregateKind: "thread",
            aggregateId: ThreadId.make("inactive"),
            occurredAt: NOW,
            commandId: null,
            causationEventId: null,
            correlationId: null,
