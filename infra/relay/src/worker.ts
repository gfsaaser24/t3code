...
export const ApiLive = Api.make(
  ...
  Effect.gen(function* () {
    ...
    // Same "no mobile push at all" condition as the two layers above: with both
    // delivery paths disabled every publish prepares deliveries that resolve to
    // null, so the push-off publisher keeps the row write and skips the prep.
    // One configured path is enough to need the real publisher, which is what
    // resolves the per-user APNs and FCM targets.
    const agentActivityPublisherLayer =
      apnsDeliveryQueueSender === null && fcmDeliveryQueueSender === null
