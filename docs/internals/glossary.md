...
| Usage window        | One quota bucket a provider account is metered against, modelled provider-agnostically as `{ id, label, usedPercent, resetsAt }`. Readings are volatile and never persisted. See [usage and limits](../user/usage.md). |
